...
$qexisting = "SELECT Record_date  from daily_detail where Record_date = '$dateshow' and detail_type = '$type'";

$existing = mysql_query($qexisting, $DB) or die(mysql_error());

$numexisting = mysql_num_rows($existing);

$qlast = "SELECT Record_date FROM daily_detail WHERE Record_date = ( SELECT MAX(Record_date) from daily_detail where detail_type = '$type') ";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
   ...
   $netdate = $_SESSION['fixdate'];
        ...
        $message = "*ข้อมูลเก่าจากวันที่ ".thai_date(strtotime($dateshow));
        $btadd = 0;
        $showdate= 1;
      
$_SESSION['newdetail'] = "newdetail";
     ...
     if ($numexisting==0) {
        ...
        $dateshow = $row_last['Record_date'];
        $message = "*ข้อมูลเก่าจากวันที่ ".thai_date(strtotime($row_last['Record_date']));
        $btadd = 0;
        $showdate= 1;
       $_SESSION['newdetail'] = "newdetail";
