...
$qmacde = "SELECT * FROM equipment_group a inner join daily_report b on a.Equipment_id = b.Equipment_id where b.TimeDate_record between '$fdate' and '$ldate' GROUP by a.Equipment_id ";

$macde      = mysql_query($qmacde, $DB) or die(mysql_error());
$nummacde   = mysql_num_rows($macde);
$row_macde = mysql_fetch_assoc($macde);
...
<body style="text-align: center; padding: 20px; background-image: url(<?php echo $_SESSION['image']; ?>);">
 ...
 <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>">
	เดือน <input type="date" required="required" name="fdate" value="<?php echo($fdate) ?>"> ปี <input type="date" required="required" name="ldate" value="<?php echo($ldate) ?>">แผนก <input type="date" required="required" name="ldate" value="<?php echo($ldate) ?>"><input type="submit" value="ดูรายงาน"></form>
...
<th width="80">ต้องการ</th>
<th width="80">มีใช้</th>
<th width="80">PM</th>
<th width="80">CM</th>
...
$qegatno = "SELECT egatno FROM equipment WHERE eqtype =".$rowdate['Equipment_type'];
